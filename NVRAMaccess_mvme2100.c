...
/*  if (bug_map.SubnetIPAddressMask != DEFAULT_SUBNETMASK) */
    ...
    if (bug_map.ArgumentFilenameString[0])
        safe_strncpy(ptr->startupScript, bug_map.ArgumentFilenameString,
            BOOT_FILE_LEN);
}
 ...
 *
 **************************************************************************-*/
static void appendString(char *buf, char *prefix, int force, char *str)
{
    if ((force == 1) || (*str != '\0'))
...
void writeNVram(BOOT_PARAMS * ptr)
{
    ...
    safe_strncpy(bug_map.BootFilenameString, ptr->bootFile,
        BUG_FILE_NAME_LEN);
    safe_strncpy(bug_map.ArgumentFilenameString, ptr->startupScript,
        BUG_FILE_NAME_LEN);
