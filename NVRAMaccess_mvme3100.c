...
static void writeGEVback(char *ptr)
{
    int                              fd;
    struct stat                      buf;
    ...
    /*
    dirty hack : 
    Unfortunally is the access to the VPD eeprom implemented as read only,
    ...
    */
    if ((fd = open(BSP_I2C_VPD_EEPROM_DEV_NAME, 0)) < 0) return;
	...
	i2c_2b_eeprom_driver_descriptor->ops->write_entry(major, minor, (void *) &rwargs);
