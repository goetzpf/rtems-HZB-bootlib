...
static int getIndex(const char *str)
{
...
#if 0
static int kompareStrList(char *str, char *strlist)
{
    char *sptr = strlist, *eptr = strchr(strlist, ',');
    int len, slen = strlen(str);
    
    if (eptr == NULL) len = strlen(strlist); else len = (eptr - sptr);
    
    while ((sptr != NULL) && (*sptr != '\0'))
    {
        if ((slen == len) && (strncmp(str, sptr, len) == 0)) return 1;
        sptr += len + 1;
        eptr = strchr(sptr, ',');
        if (eptr == NULL) len = strlen(sptr); else len = (eptr - sptr);
    }
    return 0;
}
#endif

static void freeGEVstruct(void)
{
	int i;
    for (i = 0; i < knownItems; ++i)
    {
    	if (GEVptr[i]->name != NULL) free(GEVptr[i]->name); GEVptr[i]->name = NULL;
    	if (GEVptr[i]->value != NULL) free(GEVptr[i]->value); GEVptr[i]->value = NULL;
		if (GEVptr[i]->nextitem != NULL)
        {
    		...
    		while (ptr != NULL)
    		{
    			...
    			if (ptr->value != NULL) free(ptr->value); ptr->value = NULL;
                delme = ptr;
                ptr = ptr->nextitem;
                if (delme != GEVptr[i]) free(delme);
            }
        }
...
#ifdef DEBUG
static void printGEVstruct(void)
{
	int i;
    for (i = 0; i < knownItems; ++i)
    {
    	if (GEVptr[i]->name != NULL) printf("#%i: %s=", i, GEVptr[i]->name);
    	if (GEVptr[i]->value != NULL) printf("\"%s\"\n", GEVptr[i]->value);
		if (GEVptr[i]->nextitem != NULL)
        {
        	nvpair *ptr = GEVptr[i];
    		while (ptr != NULL)
    		{
    			if (ptr->name != NULL) printf("#%i [nonregistered] %s=", i, ptr->name);
    			if (ptr->value != NULL) printf("\"%s\"\n", ptr->value);
                ptr = ptr->nextitem;
            }
        }
    }
		...
		switch (modus)
        {
            ...
            case 1: /* search value */
            	if (c == '=') /* field delimiter */
                {
                    ...
                    byteCopy((volatile char *) key, (volatile char *) ptr, (int) (nvp - ptr));
                    key[nvp - ptr] = 0;
            ...
            default: /* search values end */
            	if (c == '\0')
                {
                    ...
                    byteCopy((volatile char *) value, (volatile char *) ptr, nvp - ptr + 1);
...
static void flushGEVstruct(void)
{
    ...
    for (i = 0; i < knownItems; ++i)
    {
        ...
        {
            byteCopy(nvp, (volatile char *) GEVptr[i]->name, strlen(GEVptr[i]->name));
            nvp += strlen(GEVptr[i]->name);
            *nvp++ = '=';
            if (GEVptr[i]->value != NULL)
            {
    	        byteCopy(nvp, (volatile char *) GEVptr[i]->value, strlen(GEVptr[i]->value) + 1);
        ...
        while (ptr != NULL)
        {
            if (ptr->name != NULL)
            {
                ...
                *nvp++ = '=';
                if (ptr->value != NULL)
                {
    	            byteCopy(nvp, (volatile char *) ptr->value, strlen(ptr->value) + 1);
...
void readNVram(BOOT_PARAMS *ptr)
{
    ...
    if (GEVstruct.motscript.name != NULL) /* get boot device */
    	getsubstr(GEVstruct.motscript.value, ptr->bootDev, param_lengths[0], "-d/dev/");
    else
...
/* begin offset area */
    ...
    else getsubstr(GEVstruct.motscript.value, ptr->had, param_lengths[5], "-s");
    
    if (GEVptr[bootdev + 2]->name != NULL) /* get gateway IP */
    	strcpy(ptr->gad, GEVptr[bootdev + 2]->value);
    else getsubstr(GEVstruct.motscript.value, ptr->gad, param_lengths[6], "-g");
    
    if (GEVptr[bootdev + 3]->name != NULL) /* get subnetmask */
    	strcpy(buf, GEVptr[bootdev + 3]->value);
    else getsubstr(GEVstruct.motscript.value, buf, sizeof(buf), "-m");
    cvrtsmask(buf, buf + 1);
	buf[0] = ':';
    strcat(ptr->ead, buf);
...
/* offset area end */
    ...
    else strcpy(ptr->passwd, ""); 

    if ((GEVstruct.boot_flags.name != NULL) &&
        (GEVstruct.boot_flags.value != NULL) &&
        (strlen(GEVstruct.boot_flags.value) > 2)) /* get bootflags */
...
void writeNVram(BOOT_PARAMS *ptr)
{
    ...
    if (ptr->unitNum != 1) ptr->unitNum = 0;
...
/* begin offset area */
    ...
    if (GEVptr[bootdev]->value != NULL) free(GEVptr[bootdev]->value);
    if (cptr != NULL)
        GEVptr[bootdev]->value = strndup(ptr->ead, (int) (cptr - ptr->ead));
    else GEVptr[bootdev]->value = strdup(ptr->ead);
   
    if (GEVptr[bootdev + 1]->name == NULL) /* set host IP */
    	GEVptr[bootdev + 1]->name = strdup(GEVhash[bootdev + 1]);
    if (GEVptr[bootdev + 1]->value != NULL) free(GEVptr[bootdev + 1]->value);
    ...
    if (GEVptr[bootdev + 2]->value != NULL) free(GEVptr[bootdev + 2]->value);
    ...
    if (GEVptr[bootdev + 3]->value != NULL) free(GEVptr[bootdev + 3]->value);
    if (cptr == NULL)
    ...
    if (GEVptr[bootdev + 4]->value != NULL) free(GEVptr[bootdev + 4]->value);
...
/* offset area end */
    ...
    if (GEVstruct.client_name.value != NULL) free(GEVstruct.client_name.value);
    ...
    if (GEVstruct.epics_script.value != NULL) free(GEVstruct.epics_script.value);
    ...
    if (GEVstruct.epics_nfsmount.value != NULL) free(GEVstruct.epics_nfsmount.value);
    ...
    if (GEVstruct.rsh_user.value != NULL) free(GEVstruct.rsh_user.value);
    GEVstruct.rsh_user.value = strdup(ptr->usr);

    if (GEVstruct.tftp_pw.name == NULL) /* set PW */
    	GEVstruct.tftp_pw.name = strdup(GEVhash[19]);
    if (GEVstruct.tftp_pw.value != NULL) free(GEVstruct.tftp_pw.value);
    ...
    if (GEVstruct.host_name.value != NULL) free(GEVstruct.host_name.value);
    ...
    if (GEVstruct.motscript.value != NULL)
    {
        ...
        if (tftpline != NULL)
        {
            ...
            if ((cptr = strstr(tftpline, "-d/dev/")) != NULL)
            {
                ...
                eptr = strchr(cptr, '\0');
                if (((tptr = strchr(cptr, '\n')) != NULL) && (tptr < eptr)) eptr = tptr;
                if (((tptr = strchr(cptr, ' ')) != NULL) && (tptr < eptr)) eptr = tptr;
                ...
                } else /* we have to enlarge the buffer */
                {
                    ...
                    char *motscript = malloc(strlen(GEVstruct.motscript.value) + strlen(buf) + 1);

                    strncpy(motscript, GEVstruct.motscript.value, (int) (cptr - GEVstruct.motscript.value));
            ...
            } else script_corrupted = 1; /* this can only reached if the -d argument is missed - create new script: */
        } else script_corrupted = 1; /* how could RTEMS loaded without tftpGet command? Anyway, overwrite script! */
    }
    if ((GEVstruct.motscript.name == NULL) || (script_corrupted)) /* set new motscript */
    {
        ...
        if (GEVstruct.motscript.value != NULL) free(GEVstruct.motscript.value);
    	GEVstruct.motscript.value = malloc(strlen(part1) + strlen(buf) + strlen(part2) + 1);
